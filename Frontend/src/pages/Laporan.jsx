...
const Laporan = () => {
    ...
    transactions.forEach(transaction => {
      ...
      if (transaction.products && Array.isArray(transaction.products)) {
        transaction.products.forEach(product => {
          ...
          reportsByDate[date].totalRevenue += parseInt(product.totalRevenue || 0);
          reportsByDate[date].totalUmkmProfit += parseInt(product.umkmProfit || 0);
        });
      }
    });

    // Convert to array and sort by date (newest first)
    const reportsArray = Object.values(reportsByDate).map(report => ({
      ...report,
    ...
    if (filterPeriod !== 'semua') {
      ...
      switch (filterPeriod) {
        case 'hari-ini':
          ...
          filtered = reportsArray.filter(report => report.date === filterDate);
          break;
        case 'kemarin':
        ...
        case '7-hari':
          ...
          filtered = reportsArray.filter(report => new Date(report.date) >= sevenDaysAgo);
          break;
        case '30-hari':
          const thirtyDaysAgo = new Date(today);
          thirtyDaysAgo.setDate(today.getDate() - 30);
          thirtyDaysAgo.setHours(0, 0, 0, 0);
          filtered = reportsArray.filter(report => new Date(report.date) >= thirtyDaysAgo);
          break;
        case 'custom':
  ...
  const grandTotals = filteredReports.reduce((totals, report) => ({
          ...
          <div className="bg-white p-4 rounded shadow-sm mb-4">
            <div className="row align-items-end">
              <div className="col-md-3">
                <label htmlFor="periodeCepat" className="form-label">Periode Cepat</label>
                <select 
              ...
              {filterPeriod === 'custom' && (
                <>
                  <div className="col-md-3">
                    <label htmlFor="dariTanggal" className="form-label">Dari Tanggal</label>
                    <input
                      type="date"
                  ...
                  <div className="col-md-3">
                    <label htmlFor="sampaiTanggal" className="form-label">Sampai Tanggal</label>
                    <input
                      type="date"
                      className="form-control"
                      id="sampaiTanggal"
              ...
              <div className="col-md-3">
                <button className="btn w-100" style={{ backgroundColor: '#973131', color: '#fff' }} onClick={handleFilter}>
                  Filter
                </button>
              </div>
            </div>
              ...
              <div className="col-md-3">
                <div className="card bg-secondary text-white">
                  <div className="card-body text-center">
                    ...
                    <h4>Rp {grandTotals.totalRevenue.toLocaleString('id-ID')}</h4>
              ...
              <div className="col-md-3">
                <div className="card bg-success text-white">
                  <div className="card-body text-center">
                    ...
                    <h4>Rp {grandTotals.totalUmkmProfit.toLocaleString('id-ID')}</h4>
            ...
            {filteredReports.length > 0 && (
              <div className="p-3 border-top bg-light">
                <div className="row text-center">
                  ...
                  <div className="col-md-3">
                    <small className="text-muted d-block">Rata-rata Mitra/Hari</small>
                    <strong>{(grandTotals.totalPartners / filteredReports.length).toFixed(1)}</strong>
                  </div>
                  <div className="col-md-3">
                    <small className="text-muted d-block">Rata-rata Produk/Hari</small>
                    <strong>{Math.round(grandTotals.totalProductsSold / filteredReports.length)} pcs</strong>
                  </div>
                  <div className="col-md-3">
                    <small className="text-muted d-block">Rata-rata Keuntungan/Hari</small>
                    <strong>Rp {Math.round(grandTotals.totalUmkmProfit / filteredReports.length).toLocaleString('id-ID')}</strong>
                  </div>
                </div>
              </div>
            )}
